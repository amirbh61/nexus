...
output_macro_Mfolder="/gpfs0/arazi/users/amirbenh/Resolving_Power/nexus/SquareFiberDatabase/"
...
do
  while true; do
    ...
    if [ "$job_count" -lt "$max_jobs" ]; then
      ...
      qsub -q fairshare.q -o ~/Resolving_Power/nexus/SquareFiberCluster_outputs -e ~/Resolving_Power/nexus/SquareFiberCluster_errors job.sh "$geometry_folder" "$sub_dir"
